...
#pragma region Windows Form Designer generated code
		   ...
		   void InitializeComponent(void)
		   {
			   ...
			   for each (Object ^ var in this->Controls) {
				   if (var->GetType() == button1->GetType()) {
					   Button^ myB = (Button^)var;
					   if (gg->getFirstZero() == myB->TabIndex) {
						   myB->BackColor = System::Drawing::Color::Pink;



					   }
					   else if (myB->TabIndex >= 1 && myB->TabIndex <= 64) {
						   myB->BackColor = System::Drawing::Color::Teal;
					   }
				   }
	...
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		std::cout << "My Rem" << gg->getRem() << "  ";

		Button^ b = (Button^)sender;


		//std::cout << "i just entered " << b->TabIndex << "   ";
		//int i = b->TabIndex / 5;
		int tabi = b->TabIndex;

		int x = gg->getNodeValue(tabi - 1);
		//if (gg->getExplore(tabi - 1))return;
		bool mine = gg->checkBomb(tabi - 1);
		//sender->Text = "1";

		if (j) {
			b->BackColor = System::Drawing::Color::LightSalmon;
			//std::cout << " before mine" << b->TabIndex << "   ";
			ss++;
			if (ss % 8 == 0)std::cout << std::endl;
			if (mine) {
				b->BackColor = System::Drawing::Color::PaleVioletRed;
				b->Text = "X";
				...
				b->Text = gg->getNodeValue(tabi - 1).ToString();
				bool explored = gg->getExplore(tabi - 1);
				if (!gg->getExplore(tabi - 1)) {
					gg->explore(tabi - 1);
					if (gg->getRem() == 0) {
						b->Text = gg->getNodeValue(tabi - 1).ToString();
					...
					if (x == 0) {
						gg->exploreNodeAdj(tabi - 1);
						for each (Object ^ var in this->Controls) {
							if (var->GetType() == b->GetType()) {
								Button^ myB = (Button^)var;
								if (myB->TabIndex - 1 >= 0 && myB->TabIndex - 1 <= 63) {
									if (gg->getExplore(myB->TabIndex - 1)) {
										myB->PerformClick();
									}
				...
				if (x == 0) {
					...
					{
						if (var->GetType() == b->GetType()) {
							std::cout << " here check if button " << std::endl;
							Button^ myB = (Button^)var;
							if (myB->TabIndex - 1 >= 0 && myB->TabIndex - 1 <= 63) {
								std::cout << " here check in graph " << std::endl;
								if ((myB->TabIndex == tabi + 1 || myB->TabIndex == tabi - 1 || myB->TabIndex == tabi - 8 ||
									myB->TabIndex == tabi - 9 || myB->TabIndex == tabi - 7 || myB->TabIndex == tabi + 8 ||
									myB->TabIndex == tabi + 7 || myB->TabIndex == tabi + 9) && !gg->getExplore(myB->TabIndex - 1)) {


									std::cout << "here check of zero  " << myB->TabIndex << std::endl;
									gg->explore(myB->TabIndex - 1);
									myB->PerformClick();
								}
								else std::cout << "here is in else   " << myB->TabIndex << std::endl;
							}
						}
